  ...
  if (notFound) {
    return (
      <main className="mx-auto flex w-full max-w-xl flex-1 flex-col items-center justify-center gap-4 px-6 py-20 text-center">
        ...
        <button onClick={() => router.push('/community')} className="rounded-full border border-neutral-300 px-5 py-2 text-sm dark:border-neutral-700">목록으로</button>
      </main>
    );
  }
  ...
  return (
    <main className="mx-auto flex w-full max-w-xl flex-1 flex-col gap-6 px-6 py-12">
      <div className="rounded-2xl border border-neutral-200 p-6 dark:border-neutral-800">
        ...
        <p className="mt-5 whitespace-pre-wrap text-sm leading-relaxed text-neutral-700 dark:text-neutral-300">{post.content}</p>

        <div className="mt-6 flex items-center gap-2">
          <button onClick={() => patchPost('like')}
            className={`rounded-full border px-4 py-1.5 text-sm ${post.isLiked ? 'border-rose-400 bg-rose-50 text-rose-600 dark:bg-rose-950/40 dark:text-rose-400' : 'border-neutral-300 text-neutral-600 dark:border-neutral-700 dark:text-neutral-400'}`}>
            ❤ 좋아요 {post.likeCount}
          </button>
          {post.isMine ? (
            <button onClick={deletePostAction} className="rounded-full border border-neutral-300 px-4 py-1.5 text-sm text-neutral-600 dark:border-neutral-700 dark:text-neutral-400">삭제</button>
          ) : (
            <button onClick={() => patchPost('report')} className="rounded-full border border-neutral-300 px-4 py-1.5 text-sm text-neutral-600 dark:border-neutral-700 dark:text-neutral-400">신고</button>
          )}
        </div>
      </div>
      ...
      <div>
          ...
          {comments.map((c) => (
            <div key={c.id} className="rounded-lg border border-neutral-200 p-3 dark:border-neutral-800">
              <div className="flex items-center justify-between">
                <span className="text-xs font-semibold text-neutral-600 dark:text-neutral-300">{c.nickname}</span>
        ...
        <div className="mt-4 flex gap-2">
          <input value={commentInput} onChange={(e) => setCommentInput(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && submitComment()}
            placeholder="댓글을 입력하세요" className="flex-1 rounded-lg border border-neutral-300 px-3 py-2 text-sm dark:border-neutral-700 dark:bg-neutral-900" />
          <button onClick={submitComment} className="rounded-lg bg-neutral-900 px-4 py-2 text-sm text-white dark:bg-white dark:text-neutral-900">등록</button>
        </div>
      </div>
    </main>
  );
